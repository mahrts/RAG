...
from RAGAS import ragas_evaluator
from RAGCLIENT import rag_client
...
def display_evaluation_metrics(scores: Dict[str, float]):
    ...
    for metric_name, score in scores.items():
        ...
        if isinstance(score, (int, float)):
            is_valid = not math.isnan(score)
            safe_score = float(score) if is_valid else 0.0
...
def main():
    st.title("🚀 Hi there, this is NASA Space Mission Chat with Evaluation")
    st.markdown("Chat with AI about NASA space missions " \
    "            with an AI expert and with real-time quality evaluation")
